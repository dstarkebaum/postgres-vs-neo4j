...
do
  ...
  do
    echo "$((SECONDS-start_time)): zipping data/neo4j/s2-corpus-$i_$f.csv"
    gzip -r data/neo4j/s2-corpus-$i_$f.csv
    echo "$((SECONDS-start_time)): uploading data/neo4j/s2-corpus-$i_$f.csv.gz to S3"
    aws s3 cp data/neo4j/s2-corpus-$i_$f.csv.gz s3://data-atsume-arxiv/open-corpus/2019-09-17/neo4j/$1/$f.csv.gz
  done
